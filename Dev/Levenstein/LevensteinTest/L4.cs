...
namespace LevensteinTest
{
	unsafe class L4
	{
		...
		/// Конструктор
		/// </summary>
		public L4(int v_WordSimilarityKoeff, int v_WordExceedingKoeff, int v_max_transformations, int v_CHANGE, int v_ADDITION, int v_DELETION, int v_cuttingLimit)
		{
		...
		/// Конструктор
		/// </summary>
		public L4(int v_WordSimilatyKoeff, int v_WordExceedingKoeff, int v_CHANGE, int v_ADDITION, int v_DELETION, int v_cuttingLimit)
		...
		/// </summary>
		/// <remarks>
		/// Метод предполагает, что строки дедупликации в таблице сформированы особым образом: слова в строке расположены в порядке увеличения их длины.
		/// Это позволяет применить алгоритм, когда сравнение производится в порядке возрастания модулей для разности длин сравниваемых слов.
		/// Таким образом соответсвующее слово находится в целом быстрее. Кроме того, при ненулевом модуле разности возникает возможность выяснить,
		/// может ли потенциально данное сравнение дать результат, превышающий коэффициенты, полученные при сравнениях со словами с меньшим модулем разности.
		/// </remarks>
		...
		public unsafe bool CalculateSimilarity(char** pSourceArr, int* pSourceLenArr, int sourceArrLen, string Sample, out double SimilarityKoeff)
		{
			...
			fixed (char* pSample = Sample)
			{
				...
				// Находим слова и их длины в строке
				...
				for (int i = 0; i < sourceArrLen; i++)
				{
					...
					// Находим слово, чья длина даёт нулевой модуль разности
					int sampleWordIndex = this.GetStartingIndex(sampleWordCount, sourceWordLength);
					int leftIndex, rightIndex;
					if (sampleWordIndex >= 0) // Такое слово не найдено
					{
						leftIndex = rightIndex = sampleWordIndex;
					}
					else // Нет слов с той же длиной, что у слова в поиске
					{
					...
					while (true)
					{
						...
						{
							// Выясняем, имеет ли смысл продолжать дальнейший поиск
						...
						bool result = Levenshtein_Distance(sourceWord, sourceWordLength, pSample + pOffsetArr[currentIndex], pLenArr[currentIndex], out ldist, dist_im1);
						if (result)
						{
