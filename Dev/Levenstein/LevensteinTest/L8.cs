...
namespace LevensteinTest
{
	unsafe class L8
	{
		...
		int[] offsetList = new int[100];
		int[] lenList = new int[100];
		int[] dist_im1_array = new int[100];
		...
		/// Конструктор
		/// </summary>
		public L8(int v_WordSimilarityKoeff, int v_WordExceedingKoeff, int v_max_transformations, int v_CHANGE, int v_ADDITION, int v_DELETION, int v_cuttingLimit)
		{
		...
		/// Конструктор
		/// </summary>
		public L8(int v_WordSimilatyKoeff, int v_WordExceedingKoeff, int v_CHANGE, int v_ADDITION, int v_DELETION, int v_cuttingLimit)
			...
			fixed (byte* pbSample = sampleData)
			{
				...
				for (int i = 0; i < sourceArrLen; i++)
				{
					...
					while (true)
					{
						...
						if (sourceWordLength != pLenArr[currentIndex])
						{
							if (KoeffTemp > 0)
							{
						...
						else if (CompareStrings(sourceWord, pSample + pOffsetArr[currentIndex], sourceWordLength))
						{
							KoeffTemp = 1;
							break;
						}

						int outerldist = (KoeffTemp > 0 ? (int)Math.Ceiling(temp1 / KoeffTemp - temp1 + 0.1) - 1 : int.MaxValue);

						if (!Levenshtein_Distance(sourceWord, sourceWordLength, pSample + pOffsetArr[currentIndex], pLenArr[currentIndex], outerldist, out ldist))
						{
							continue;
						}
		...
		private unsafe bool Levenshtein_Distance(char* sourceWord, int sourceWordLength, char* Sample, int sampleLen, int outerldist, out int ldist)
		{
			ldist = 0;
			...
			if (abs(ref sourceWordLength, ref sampleLen) <= max_transformations)
				if ((ldist = l_dist_raw(sourceWord, sourceWordLength, Sample, sampleLen, outerldist)) <= max_transformations)
					return (true);

			return (false);
		}

		private unsafe int l_dist_raw(char* sourceWord, int sourceWordLength, char* Sample, int sampleLen, int outerldist)
		{
			int i, j;
			...
			for (i = 0; i < sourceWordLength; i++)
			{
				...
				if (MinDist - (sourceWordLength - 1 - i) >= outerldist)
				{
					return outerldist;
				}
		...
		[TargetedPatchingOptOut("speed")]
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		static unsafe bool CompareStrings(char* str1, char* str2, int length)
		{
