...
namespace LevensteinTestCore
{
	unsafe class L6
	{
		...
		/// Конструктор
		/// </summary>
		public L6(int v_WordSimilarityKoeff, int v_WordExceedingKoeff, int v_max_transformations, int v_CHANGE, int v_ADDITION, int v_DELETION, int v_cuttingLimit)
		{
		...
		/// Конструктор
		/// </summary>
		public L6(int v_WordSimilatyKoeff, int v_WordExceedingKoeff, int v_CHANGE, int v_ADDITION, int v_DELETION, int v_cuttingLimit)
		...
		public unsafe bool CalculateSimilarity(char** pSourceArr, int* pSourceLenArr, int sourceArrLen, string Sample, out double SimilarityKoeff)
		{
			...
			fixed (char* pSample = Sample)
			{
				...
				this.ParseString(Sample, out sampleWordCount, out maxSampleWordLength);
		...
		//[TargetedPatchingOptOut("speed")]
		//[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private unsafe void ParseString(string sample, out int wordCount, out int maxWordLen)
		{
			maxWordLen = 0;
			wordCount = 0;
			...
			int spaceOnSlice = 0;
			ReadOnlySpan<char> any = " \t\n\r";
			ReadOnlySpan<char> s = sample;
			do
			{
				slice = s.Slice(currentWordStart);
				spaceOnSlice = slice.IndexOfAny(any);

				if (spaceOnSlice < 0)
				{
					spaceOnSlice = slice.Length;
				}
				...
				pOffsetArr[wordCount] = currentWordStart;
				pLenArr[wordCount] = spaceOnSlice;
				currentWordStart = currentWordStart + spaceOnSlice + 1;
				wordCount++;
			}
			while (slice.Length > spaceOnSlice);
		}
		...
		private unsafe bool Levenshtein_Distance(char* sourceWord, int sourceWordLength, char* Sample, int sampleLen, out int ldist)
		{
			...
			if (abs(sourceWordLength, sampleLen) <= max_transformations)
