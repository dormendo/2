...
namespace TcpTest
{
	class Program
	{
		static int _started = 0, _connected = 0, _sent = 0, _received = 0, _closed = 0, _errors = 0, _iterations = 0, _port;
		static string _host;
		static async Task Main(string[] args)
		{
			...
			string requestStr = File.ReadAllText("Request.txt");
			byte[] request = Encoding.UTF8.GetBytes(requestStr);
			headersStr = headersStr.Replace("{LENGTH}", request.Length.ToString());
