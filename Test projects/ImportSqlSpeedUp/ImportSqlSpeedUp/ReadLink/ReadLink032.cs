...
{
	class ReadLink032 : ReadLinkTestBase
	{
		...
		public ReadLink032(string name, int iterationCount, int rowCount, string connString)
		...
		protected override void RunIteration(Stopwatch sw)
		{
			...
			for (int i = 0; i < codes.Count; i++)
			{
				...
				sb.Append("(N'").Append(codes[i].Replace("'", "''")).Append("', '").Append(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")).Append("', ").Append(i).Append(")");
			}

			sb.Append(") a (UserCode, DocumentBeginDate, RowIndex) ON e.UserCode = a.UserCode AND CONVERT(datetime, a.DocumentBeginDate, 120) BETWEEN e.DocumentBeginDate AND e.DocumentEndDate WHERE e.State = 0");
