...
{
	class ReadLinkOra032 : ReadLinkOraTestBase
	{
		...
		public ReadLinkOra032(string name, int iterationCount, int rowCount, string connString)
		...
		protected override void RunIteration(Stopwatch sw)
		{
			...
			for (int i = 0; i < codes.Count; i++)
			{
				...
				sb.Append("SELECT N'").Append(codes[i].Replace("'", "''")).Append("' UserCode, '").Append(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")).Append("' DocumentBeginDate, ").Append(i).Append(" RowIndex FROM dual");
			}

			sb.Append(") a ON e.UserCode = a.UserCode AND TO_DATE(a.DocumentBeginDate, 'YYYY-MM-dd HH24:MI:SS') BETWEEN e.DocumentBeginDate AND e.DocumentEndDate WHERE e.State = 0");
