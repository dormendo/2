...
{
	class SaveNologgingOra : SaveOraTestBase
	{
		...
		OracleTransaction _tx;

		int _id = 0;

		public SaveNologgingOra(string name, int iterationCount, string connString, bool prepareCommands) : base(name, iterationCount, connString)
		{
			...
			this._saveDedup = new OracleCommand("INSERT /*+ append */ INTO IT_4(ID, DEDUP_STR_1, DEDUP_STR_2, DEDUP_STR_3) VALUES (:inserted_id, :DEDUP_STR_1, :DEDUP_STR_2, :DEDUP_STR_3)", this._conn);
		...
		protected override void RunIteration(Stopwatch sw)
		{
			if (this._tx == null)
			{
				this._tx = this._conn.BeginTransaction();
				this._savePosition.Transaction = this._tx;
				this._saveDedup.Transaction = this._tx;
			}
