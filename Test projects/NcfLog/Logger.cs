using System;
using Maxima.BookmakerPrototype.Log;
...
namespace Maxima.BookmakerPrototype
{
    ...
    public static class Logger
    {
        ...
        /// </summary>
        private static readonly ILogEngine[] Loggers = new ILogEngine[2];

        private static readonly NcfLogEngineProvider NcfLeProvider;

        static Logger()
        {
            NcfLeProvider = new NcfLogEngineProvider();
            NcfLeProvider.Initialize();

#if DEBUG
            RegisterLog(LogType.System, "SystemDev");
#else
            RegisterLog(LogType.System, "SystemProduction");
#endif
        }
        ...
        {
            if (Loggers[(int)type] == null)
            {
                ILogEngine logger = NcfLeProvider.GetLogger(logConfiguration);
                if (logger == null)
                {
