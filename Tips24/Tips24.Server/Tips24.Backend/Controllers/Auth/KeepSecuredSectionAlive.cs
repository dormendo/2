...
{
	public class KeepSecuredSessionAlive : AuthHandler
	{
		public KeepSecuredSessionAlive(SqlServer sqlServer, IHostingEnvironment he)
			: base(sqlServer, he)
		{
		}

		public async Task<IActionResult> Handle(HttpRequest request, KeepSsAliveRequest data)
		{
			...
			try
			{
				using (SqlConnection conn = sqlServer.GetConnection())
				{
					...
					using (SqlCommand cmd = sqlServer.GetSpCommand("dbo.Employee_KeepSecuredSessionAlive", conn))
					{
		...
		private ErrorResponse GetErrorResponse(int retVal)
		{
			...
			{
				return this.GetSecuredSessionExpiredError();
			}
		}
	}
}
