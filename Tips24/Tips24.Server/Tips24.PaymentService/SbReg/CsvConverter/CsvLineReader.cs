...
using System.Text;
using System.IO;
using Tips24.PaymentService.Properties;
...
namespace Tips24.PaymentService.SbReg.Csv
{
	...
	public class CsvLineReader
	{
		...
		public List<string> ReadAndValidate()
		{
			...
			{
				throw new Exception(string.Format(Resources.CharOutsideQuote, result.LineNumber, result.LinePosition));
			}
			else if (result.FieldResult == CsvReadFieldResult.QuoteInNqField)
			{
				throw new Exception(string.Format(Resources.UnexpectedQuote, result.LineNumber, result.LinePosition));
			}
			else if (result.FieldResult == CsvReadFieldResult.QuoteNotFound)
			{
				throw new Exception(string.Format(Resources.LineEndsWithInvalidQuotedField, result.LineNumber, result.LinePosition));
			}
			else
			{
				return null;
